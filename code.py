...
u_converter = Tk()
u_converter.title("Calculator")
u_converter.geometry('250x220')
u_converter.resizable(0, 0)

# Adding logo
# path = "logos.ico"
# load = Image.open(path)
# render = ImageTk.PhotoImage(load)
# u_converter.iconphoto(False, render)

def unitconverter():
    """ Check logo and background image code """
    ...
    # bag = Canvas(unitConverter, width=450, height=305)
    # bag.pack(fill="both", expand=True)
    # bag.create_image(0, 0, image=bg, anchor="nw")
        ...
        def result():
            ...
            try:
                ...
                try:
                    ...
                    messagebox.showinfo('Alert!', f"Wrong Input, try again!!")
...
def financialconverter():
    pass


Label(u_converter, text="Welcome!", font=("Arial Bold", 17), justify='center').pack(pady=10)
Button(u_converter, text="Basic Calculator", padx=22, command=basiccalculator).pack(pady=1)
Button(u_converter, text="Unit Converter", padx=27, command=unitconverter).pack(pady=1)
Button(u_converter, text="Financial Converter", command=financialconverter).pack(pady=1)
Button(u_converter, text="Close", command=u_converter.destroy).pack(pady=10)

u_converter.mainloop()
